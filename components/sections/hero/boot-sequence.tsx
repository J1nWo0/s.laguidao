...
export function BootSequence({ children }: { children: React.ReactNode }) {
  ...
  return <BootSequenceProvider value={value}>{children}</BootSequenceProvider>;
}
...
export function BootPrompt({ step, command, caret = false, className }: BootPromptProps) {
  ...
  return (
    <p
      ...
      className={cn("reveal flex flex-wrap items-baseline gap-x-2 text-sm", className)}
    >
      <PromptChrome />
      ...
      {command ? (
        <span className="text-foreground">
          ...
          {showCaret ? <Caret steady={typing} className="ml-0.5" /> : null}
        </span>
      ) : showCaret ? (
        <Caret steady={typing} />
      ) : null}
